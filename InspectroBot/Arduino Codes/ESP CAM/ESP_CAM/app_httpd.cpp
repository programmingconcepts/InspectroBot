...
void startCameraServer() {
  ...
  // New route for controlling the bot
  httpd_uri_t control_bot_uri = {
    .uri = "/controlBot",
    .method = HTTP_GET,
    .handler = [](httpd_req_t *req) {
      ...
      if (httpd_req_get_url_query_str(req, data, sizeof(data)) == ESP_OK) {
        httpd_query_key_value(data, "direction", direction, sizeof(direction));
        httpd_query_key_value(data, "speed", speed, sizeof(speed));
        String response = String(direction) + String(speed);
        Serial.println(response);  // Send data to Arduino over Serial
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &control_bot_uri);
  }
